        ...
        <div class="col-md-10 col-md-offset-2">
            <div class="col-md-12">
                <ol class="breadcrumb">
                    ...
                    <c:if test="${!empty condition}">
                        <li><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=1&size=${pageInfo.pageSize}">查询全部</a></li>
        ...
        <table class="table table-hover table-inner-border" style="table-layout: fixed;width: 100%;">
            ...
            <c:forEach items="${pageInfo.list}" var="userInfo" varStatus="s">
                <tr>
                    ...
                    <td>
                        <button class="btn-sm btn-success" onclick="window.location.href = '${pageContext.request.contextPath}/borrow/saveNowBorrowUser.do?id=${userInfo.id}&name=${userInfo.realName}'">选择</button>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
...
<nav class="navbar-fixed-bottom" style="text-align:center;margin-bottom: 15px" >
    <ul class="pagination">
        <%--<li><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=1&size=${pageInfo.pageSize}&condition=${condition}">首页</a></li>--%>
        <li><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}&condition=${condition}"  aria-label="Previous">&laquo;</a></li>
        <%--如果长度小于7，直接完整显示--%>
        <c:if test="${pageInfo.pages <= 7}">
            <c:forEach begin="1" end="${pageInfo.pages}" var="pageNum">
                <li id="page_${pageNum}"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageNum}&size=${pageInfo.pageSize}&condition=${condition}">${pageNum}</a></li>
            </c:forEach>
        </c:if>
        ...
        <c:if test="${pageInfo.pages > 7}">
            ...
            <c:if test="${((pageInfo.pageNum - 1) < 4) && ((pageInfo.pageNum - 1) <= (pageInfo.pages - pageInfo.pageNum))}">
                ...
                <li><a href="#">...</a></li>
                <li id="page_${pageInfo.pages}"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageNum}&size=${pageInfo.pageSize}&condition=${condition}">${pageInfo.pages}</a></li>
            </c:if>
            <%--更靠近尾页，且当前页离尾页只有不到3格--%>
            <c:if test="${((pageInfo.pages - pageInfo.pageNum) < 4) && ((pageInfo.pageNum - 1) > (pageInfo.pages - pageInfo.pageNum))}">
                <li id="page_1"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageNum}&size=${pageInfo.pageSize}&condition=${condition}">1</a></li>
                <li><a href="#">...</a></li>
            ...
            <c:if test="${(pageInfo.pageNum - 1 >= 4) && ((pageInfo.pages - pageInfo.pageNum) >= 4)}">
                <li id="page_1"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=1&size=${pageInfo.pageSize}&condition=${condition}">1</a></li>
                <li><a href="#">...</a></li>
                <c:forEach begin="${pageInfo.pageNum - 2}" end="${pageInfo.pageNum + 2}" var="pageNum">
                    <li id="page_${pageNum}"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageNum}&size=${pageInfo.pageSize}&condition=${condition}">${pageNum}</a></li>
                </c:forEach>
                <li><a href="#">...</a></li>
                <li id="page_${pageInfo.pages}"><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageInfo.pages}&size=${pageInfo.pageSize}&condition=${condition}">${pageInfo.pages}</a></li>
            </c:if>
        </c:if>
        <li><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}&condition=${condition}" aria-label="Next">&raquo;</a></li>
        <%--<li><a href="${pageContext.request.contextPath}/borrow/findUser.do?page=${pageInfo.pages}&size=${pageInfo.pageSize}&condition=${condition}">尾页</a></li>--%>
    </ul>
</nav>
...
<script type="text/javascript">
    ...
    function changePageSize() {
        ...
        //向服务器发送请求，改变每页显示条数
        location.href = "${pageContext.request.contextPath}/borrow/findUser.do?page=1&size="
    ...
    $(function (){
        if (${!empty condition}){
            ...
            $("#condition").attr("placeholder","请添加要进行借书操作的用户，按学号或用户名查询")
        }
    });

    function findByCondition(){
        if($("#condition").val() == ""){
            location.href = "${pageContext.request.contextPath}/borrow/findUser.do?page=1&size=${pageInfo.pageSize}"
        } else{
